% M-File  A1_8_4b.M,  Kap.8, Aufgabe 4b 
...
disp(' ')
ytab=input('16+1 Tabellenwerte in Form eines Zeilenvektors eingeben   ytab =  ');
disp(' ')
if length(ytab)~=17
   error('length of ytab is not 17')
end
disp(' '),disp(' ')
...
% ytab darstellen
subplot(3,1,2),stairs(0:16,ytab),axis([0 16.2 1.1*min(ytab) 1.1*max(ytab)])
ylabel('ytab')
...
for n=1:N,
   i=floor(8*x(n))+9;
   y(n)=ytab(i)+(ytab(i+1)-ytab(i))/.125*(x(n)-xi(i));
end
