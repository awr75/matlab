function [sos,g] = zp2sosI(varargin)
%ZP2SOSI Zero-pole-gain to second-order sections model conversion.
%
%    Same as zp2sos, but uses direct form I cascade blocks (see Lit.[4]):
%
%   -----b-Q------ ... ---b-Q--------
%      |   |   |        |   |   |
%      D-b-O-a-D        D-b-O-a-D       Q = Summation and Quantization Point
%      |   |   |        |   |   |       O = Summation Point
%      D-b-O-a-D        D-b-O-a-D       D = Delay
%
%   [SOS,G] = ZP2SOS1(Z,P,K) finds a matrix SOS in second-order sections 
...
%   G is not specified, the gain is embedded in the first section if 
%   |G| > 1 else G is embedded in the last Section. 
%
%   The second order structure thus describes the system H(z) as:
%       H(z) = G*H1(z)*H2(z)*....*HL(z)    if |G| >= 1
%       H(z) =   H1(z)*H2(z)*...G*HL(z)    if |G| <= 1 
%
%   ZP2SOSI(Z,P,K,DIR_FLAG) specifies the ordering of the 2nd order
...
%
%   ZP2SOSI(Z,P,K,DIR_FLAG,SCALE) specifies the desired scaling of the gain
...
%
%   See also TF2SOSI, SOS2ZP, SOS2TF, SOS2SS, SS2SOS, CPLXPAIR.
%
%   Example 
%   -------
%   [b,a]=ellip(6,1,40,.2); [z,p,k]=tf2zp(b,a);
%   sos=zp2sosI(z,p,k,'up','inf')
...
%     [4] D.C. von Grünigen, Digitale Signalverarbeitung
%              Fachbuchverlag Leipzig, 2002, Kapitel 6.

%   Author(s): R. Losada 
%   Modified:  I. Oesch 19.10.1999 
%   Modified:  I. Oesch 20.12.2001 if gain is > 1, embed in first, else embed in last 
...
if nargout == 1,
   % if gain is > 1, embed in first, else embed in last 
   if abs(k) >= 1
      sos(1,1:3) = k*sos(1,1:3);
   else
      sos(end,1:3) = k*sos(end,1:3);
   end   
else
   g = k;
end
...
sp = 1;
for m = 1:L
   Fnum = conv(Fnum,sos(m,1:3));
   Fden = conv(Fden,sos(m,4:6));
   if strmatch(lower(scale),'inf'),
      F = freqz(Fnum,Fden,256);
      s(m) = max(abs(F))./sp;
      sp = sp * s(m);
   elseif strmatch(lower(scale),'two'),
      ...
      rho = max(abs(roots(Fden)));
      neff = logtol/log(rho); % Again, this is an approximation of the effective time it takes the impulse response to decay
      H = impz(Fnum,Fden,neff);
      s(m) = norm(H)./sp;
      sp = sp * s(m);
   end
   % Now perform the scaling for the first L sos sections
   sos(m,1:3) = sos(m,1:3) ./ s(m);
end
% And now calculate the corrected overall scalefactor
g = k*prod(s);
